...
def compose_videos(
    ...
    *,
    ...
    if concat_mode == "sequential":
        ...
        # sequential 模式下，positions/preview 無效
        positions = None
    else:
        # overlay：維持原本行為
        if not positions and not preview:
            # 預設橫向並排
            total_w = sum(w for w,h in sizes)
            ...
            for w,h in sizes:
                ...
                x_offset += w
            canvas_size = (total_w, max_h)
        else:
            # 若指定 positions 或 preview 模式，先估算畫布上限
            total_w = sum(w for w,h in sizes)
            total_h = sum(h for w,h in sizes)
            max_w = max(total_w, max(w + (positions[i][0] if positions else 0) for i,(w,h) in enumerate(sizes)))
            max_h = max(total_h, max(h + (positions[i][1] if positions else 0) for i,(w,h) in enumerate(sizes)))
            canvas_size = (max_w, max_h)
 

    # 5. 若開啟互動預覽，要求使用者調整
    if preview and concat_mode == "overlay":
        # init_positions: 如果一開始沒指定，就先排橫向
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--preview', action='store_true',
                        help='啟用互動式預覽來調整各影片位置，按 s 確定後執行合併')
    ...
    parser.add_argument('-r', '--reverse', action='store_true',help='啟用倒轉輸出：合成每幀後由後往前的索引順序寫入，不需緩存整段影片')
...
C:/Users/user/miniconda3/envs/ttgui-env/python.exe d:/Rontgen/TrafficTrackerGUI-MOTC/Model/tool/videoCliper.py `
-i 'd:\Rontgen\2025\新北市中和區中山路三段_環球購物商場\A\新北市中和區中山路三段_環球購物商場_A.MP4【14642------】_stab.avi' `
-o 'd:\Rontgen\2025\新北市中和區中山路三段_環球購物商場\A\新北市中和區中山路三段_環球購物商場_A.MP4【14612------】_stab.avi' `
